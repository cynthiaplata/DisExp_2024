...
Bloq <- gl(4, 4, 16, labels = c("bajo", "normal", "sobrepeso", "obesidad"))

Dietas <- data.frame(Trat, Bloq, GP)
head(Dietas)


boxplot(Dietas$GP ~ Dietas$Trat, col = "lightblue", xlab = "Dietas", ylab = "Ganancia en peso (kg)")
...
boxplot(Dietas$GP ~ Dietas$Bloq, col = "red", xlab = "dietas", ylab = "bloques")
